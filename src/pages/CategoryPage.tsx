...
type SortOption = 'default' | 'difficulty-asc' | 'difficulty-desc' | 'name-asc' | 'name-desc';
...
const CategoryPage: React.FC = () => {
  ...
  const filteredQuestions = questions.filter((question) => {
    // Apply search filter
    if (searchQuery && !question.problem.toLowerCase().includes(searchQuery.toLowerCase())) {
      return false;
    }
  ...
  const solvedCount = category?.questions.filter((q) => getQuestionStatus(q.id)).length || 0;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 pt-24">
      <div className="mb-8">
        <Link to="/" className="text-primary-600 dark:text-primary-400 hover:underline flex items-center text-sm">
          <ArrowLeft size={16} className="mr-1" />
          Back to Categories
        </Link>
      </div>
